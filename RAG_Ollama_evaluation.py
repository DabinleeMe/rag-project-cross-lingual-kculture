...
5 configurations (each changes ONE dial from baseline):
  baseline        fixed   | cosine | STRONG model
  exp1_largechunk large   |        |
  exp2_mmr                | mmr    |
  exp3_hyde               | (hyde) |
  exp4_weakmodel          |        | WEAK model     <- generator effect
...
Output: RAG_Kculture_result.csv
...
OUT_CSV = ROOT / "RAG_Kculture_result.csv"
...
# ---- 5 configurations (language is per-question, so no query_lang here) ----
configs = [
    {"name": "baseline",        "chunking": "fixed", "search": "similarity", "hyde": False, "model": STRONG_MODEL},
    {"name": "exp1_largechunk", "chunking": "large", "search": "similarity", "hyde": False, "model": STRONG_MODEL},
    {"name": "exp2_mmr",        "chunking": "fixed", "search": "mmr",        "hyde": False, "model": STRONG_MODEL},
    {"name": "exp3_hyde",       "chunking": "fixed", "search": "similarity", "hyde": True,  "model": STRONG_MODEL},
    {"name": "exp4_weakmodel",  "chunking": "fixed", "search": "similarity", "hyde": False, "model": WEAK_MODEL},
]
...
# grounding-friendly prompt: encourages abstention instead of hallucination
...
for cfg in configs:
    ...
    for q in questions:
        for lang in q.get("languages", ["en"]):
            ...
            rag_answer = llm.invoke(PROMPT.format(context=context, question=query))
